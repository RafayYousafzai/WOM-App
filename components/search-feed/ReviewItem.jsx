...
import ListReviewCard from "../dynamic-cards/ListReviewCard";
...
const ReviewsList = ({ limit = 20 }) => {
  ...
  // State variables for managing reviews, loading status, and errors
  ...
  // Asynchronous function to fetch reviews from Supabase
  ...
  const fetchReviews = async (attemptCountryFilter = false) => {
    ...
    try {
      ...
      console.log("ReviewsList - Blocked User IDs:", blockedUserIds);

      // Initialize the Supabase query for the 'reviews' table
      let query = supabase
        .from("reviews")
        .select(
          `*, 
      ...
      if (trimmedQuery) {
        // Use an 'OR' condition to search across restaurant name, address, review text, and dish name
        // '%${trimmedQuery}%' creates a case-insensitive partial match
        query = query.or(
          `restaurant_name.ilike.%${trimmedQuery}%,` +
            `location->>address.ilike.%${trimmedQuery}%,` +
      ...
      let userFriendlyMessage =
        "Sorry, we couldn't load the reviews at the moment. Please try again later.";

      // Customize error message based on common error types
      if (err.message) {
        if (
          err.message.includes(" relación «reviews» no existe") || // Spanish error message
          err.message
            .toLowerCase()
            .includes('relation "reviews" does not exist') // English error message
        ) {
          userFriendlyMessage =
            "Error: The reviews data source seems to be missing. Please contact support.";
        } else if (err.message.includes("permission denied")) {
          userFriendlyMessage =
            "Error: We don't have permission to access the reviews. Please contact support.";
        } else if (
  ...
  // Render the ListReviewCard component with fetched posts
  return <ListReviewCard posts={posts} loading={loading} />;
};
