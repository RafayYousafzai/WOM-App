...
export const useBookmarks = () => {
  ...
  // Deletes a collection and all its bookmarks
  const deleteCollection = async (userId, collectionName) => {
    try {
...
`
  ...
  // Removes all bookmarks for a specific post by a user
  const removeAllBookmarksForPost = async ({ userId, postId }) => {
    try {
      ...
      console.error("Error removing bookmarks for post:", error);
      throw error;
    }
  };
  ...
  return {
    getUserCollections,
    createCollection,
    deleteCollection, // New function for deleting collections
