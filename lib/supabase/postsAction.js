// 🔹 POSTS - New unified post system with Gatekeeping
...
export const togglePostGatekeeping = async (supabase, postId, userId) => {
  try {
    ...
    // Toggle gatekeeping (null or false becomes true, true becomes false)
    const newGatekeepingValue = !post.gatekeeping;
    ...
    return {
      success: true,
      gatekeeping: data.gatekeeping,
      message: `Gatekeeping ${
        data.gatekeeping ? "enabled" : "disabled"
      } successfully`,
    };
  } catch (error) {
...
export const getPostGatekeepingStatus = async (supabase, postId) => {
  try {
    ...
    return {
      success: true,
      gatekeeping: data.gatekeeping === true, // null or false becomes false
    };
  } catch (error) {
