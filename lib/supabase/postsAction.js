...
export const toggleGlobalGatekeeping = async (
  supabase,
  userId,
  enableGatekeeping,
) => {
  try {
    ...
    console.log(
      `Setting global gatekeeping to ${enableGatekeeping} for user ${userId}`,
    );
...
export const getUserGatekeepingStatus = async (supabase, userId) => {
  try {
    ...
    const gatekeepingPosts = data.filter(
      (post) => post.gatekeeping === true,
    ).length;
